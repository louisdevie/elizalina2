...
class ElzLocale<T> {
  ...
  private readonly _messages: LocaleModuleLoader<T>
  ...
  public async load(factory: FormatterFactory) {
    ...
    const fmt = new Fmt(new FormatImpl(this._id), factory)
    this._instance = new module.default(fmt)
  }
...
export class Elz<T> {
  ...
  private async changeLocaleTo(locales: string[]) {
    ...
    if (chosen === undefined) {
      throw new Error(
        '[elzii] could not find any suitable locale. consider adding a global fallback.',
      )
    }
  ...
  public async useMainLocale() {
    if (this._main === undefined) throw new Error('[elzii] there is no main locale configured.')
